...
export async function getCourseProgramming(req, res) {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error al cargar las programaciones", error });
  }
}
...
export async function createCourseProgramming(req, res) {
  try {
    ...
    if (!Array.isArray(levels) || levels.length < 3) {
      return res
        .status(400)
        .json({ message: "Debe incluir al menos 3 niveles." });
    }
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (!level.name || level.name.trim() === "") {
        return res
          .status(400)
          .json({ message: `El nivel ${i + 1} debe tener un nombre.` });
      }
    }
    ...
    if (!Array.isArray(firstLevel.topics) || firstLevel.topics.length === 0) {
      return res
        .status(400)
        .json({ message: "El primer nivel debe contener al menos un tema." });
    }

    const topicSum = firstLevel.topics.reduce(
      (sum, t) => sum + (t.value || 0),
      0
    );
    if (topicSum !== 100) {
    ...
    for (const topic of firstLevel.topics) {
      if (!topic.topicId || topic.value == null) {
        return res
          .status(400)
          .json({ message: "Cada tema debe tener un ID y un valor." });
      }

      // Busca los datos del tema
      const topicDoc = validTopics.find(
        (t) => t._id.toString() === topic.topicId.toString()
      );
      if (!topicDoc) {
        return res
          .status(400)
          .json({ message: `El tema ${topic.topicId} no existe.` });
      }

      // Agrega nombre y descripción
      topic.name = topicDoc.name;
      topic.description = topicDoc.description;
      ...
      if (!Array.isArray(topic.activities) || topic.activities.length === 0) {
        return res
          .status(400)
          .json({ message: "Cada tema debe tener al menos una actividad." });
      }
      const actSum = topic.activities.reduce(
        (sum, a) => sum + (a.value || 0),
        0
      );
      if (actSum !== 100) {
      ...
      for (const activity of topic.activities) {
        if (!activity.evaluationId || activity.value == null) {
          return res
            .status(400)
            .json({ message: "Cada actividad debe tener un ID y un valor." });
        }
      ...
      if (!Array.isArray(topic.exams) || topic.exams.length === 0) {
        return res
          .status(400)
          .json({ message: "Cada tema debe tener al menos un examen." });
      }
      ...
      for (const exam of topic.exams) {
        if (!exam.evaluationId || exam.value == null) {
          return res
            .status(400)
            .json({ message: "Cada examen debe tener un ID y un valor." });
        }
      ...
      if (!Array.isArray(topic.materials) || topic.materials.length === 0) {
        return res
          .status(400)
          .json({ message: "Cada tema debe tener al menos un material." });
      }
      for (const material of topic.materials) {
        if (!material.materialId) {
          return res
            .status(400)
            .json({ message: "Cada material debe tener un ID." });
        }
    ...
    const missingTopics = allTopicIds.filter(
      (id) => !validTopics.some((t) => t._id.toString() === id.toString())
    );
    ...
    const invalidExams = allExamIds.filter(
      (id) =>
        !validEvaluations.some(
          (e) =>
            e._id.toString() === id.toString() && e.tipoEvaluacion === "Examen"
        )
    );
...
export async function updateCourseProgramming(req, res) {
  try {
    ...
    // ✅ Aquí se rellena name y description
    for (const level of levels) {
      for (const topic of level.topics) {
        const topicDoc = validTopics.find(t => t._id.toString() === topic.topicId.toString());
        if (!topicDoc) {
          return res.status(400).json({ message: `Tema ${topic.topicId} no existe.` });
        }
        topic.name = topicDoc.name;
        topic.description = topicDoc.description;
      }
    ...
    console.error(error);
    return res.status(500).json({ message: 'Error al actualizar la programación.', error: error.message });
  }
}
...
export async function deleteCourseProgramming(req, res) {
  try {
    const deleteCourseProgramming = await CourseProgramming.findByIdAndDelete(
      req.params.id
    );
    ...
    res
      .status(500)
      .json({ message: "Error al eliminar la programación", error });
  }
}
